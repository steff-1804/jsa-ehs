...
def _join_controls(peligros, field):
    ...
    for i, peligro in enumerate(peligros, start=1):
        text = (peligro.get(field) or "").strip()
...
def _insert_image(ws, fotos, anchor_cell, start_row, end_row):
    ...
    try:
        ...
        ws.column_dimensions["B"].width = max(
            ws.column_dimensions["B"].width or 18,
            min(new_w / 7, 78)
        )
        ...
        for row in range(start_row, end_row + 1):
            ws.row_dimensions[row].height = height_each
...
def exportar_excel():
    try:
        ...
        for actividad in actividades:
            peligros = actividad.get("peligros", []) or []
            ...
            for index, peligro in enumerate(peligros):
                sev = _safe_int(peligro.get("sev"), 1)
                likl = _safe_int(peligro.get("likl"), 1)
                cont = _safe_int(peligro.get("cont"), 1)
                rpn = sev * likl * cont

                sev_post = _safe_int(peligro.get("sev_post"), 1)
                likl_post = _safe_int(peligro.get("likl_post"), 1)
                cont_post = _safe_int(peligro.get("cont_post"), 1)
                rpn_post = sev_post * likl_post * cont_post

                values = [
                    actividad.get("actividad", "") if index == 0 else "",
                    "",
                    peligro.get("peligro", ""),
                    peligro.get("consecuencia", ""),
                    sev,
                    likl,
                    existing_merged if index == 0 else "",
                    cont,
                    rpn,
                    recommended_merged if index == 0 else "",
                    sev_post,
                    likl_post,
                    cont_post,
                    rpn_post,
